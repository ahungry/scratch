...
The `home-routes` handler in the `micro-graphql.routes.home` namespace
...
The `home-page` function will in turn call the `micro-graphql.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `micro-graphql.handler` namespace:
...
Request middleware functions are located under the `micro-graphql.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `micro-graphql.dev-middleware` namespace found in
